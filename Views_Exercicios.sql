...
placa CHAR(7) NOT NULL
PRIMARY KEY (codigo)
FOREIGN KEY(codigom) REFERENCES motorista(codigom),
FOREIGN KEY(placa) REFERENCES onibus(placa)
)
GO 
INSERT INTO motorista (codigom, nome, naturalidade)
VALUES
...
GO 
INSERT INTO viagem (codigo, placa, codigom, hora_saida, hora_chegada, partida, destino)
VALUES
...
SELECT CAST(codigom AS VARCHAR(255)) AS ID, Nome
FROM Motorista
UNION
SELECT placa, marca AS Nome
FROM onibus
...
SELECT
    V.codigo AS codigo_viagem,
    M.Nome AS Nome_do_Motorista,
    CONCAT(SUBSTRING(o.placa, 1, 3), '-', SUBSTRING(o.placa, 4, 4)) AS placa_onibus,
    O.marca AS marca_onibus,
    O.Ano AS ano_onibus,
    O.descricao AS descrição_onibus
FROM
    viagem V
    JOIN motorista M ON v.codigom = m.codigom
    JOIN onibus O ON v.placa = o.placa
...
SELECT
    V.codigo AS Código_da_Viagem,
    CONCAT(SUBSTRING(O.placa, 1, 3), '-', SUBSTRING(o.Placa, 4, 4)) AS Placa_onibus,
    CONCAT(V.hora_saida, ':00') AS Hora_saida,
    CONCAT(V.hora_chegada, ':00') AS Hora_Chegada,
    V.Partida,
    V.Destino
FROM
    Viagem V
